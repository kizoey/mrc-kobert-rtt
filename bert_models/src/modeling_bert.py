...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # self.num_ner_labels = 20
        self.num_pp_labels = 2
        self.num_cls_labels = 2
        # self.classifier = nn.Linear(config.hidden_size, self.num_ner_labels) # NER Labels
        self.classifier = nn.Linear(config.hidden_size, self.num_pp_labels) # PP Labels
        self.cls_classifier = nn.Linear(config.hidden_size, self.num_cls_labels) # PP Labels
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

        self.init_weights()

    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None, inputs_embeds=None,
                start_positions=None, end_positions=None, label_ids=None, shuffled_input_ids=None, shuffled_attention_mask=None,
                shuffled_token_type_ids=None, cls_labels=None):
        ...
        if label_ids is not None:
            ...
            if attention_mask is not None:
                ...
                ner_loss = loss_fct(ner_logits.view(-1, self.num_ner_labels), label_ids.view(-1))

        # shuffle Classification
        if cls_labels is not None:
            shuffled_outputs = self.bert(shuffled_input_ids,
            ...
            shuffle_labels = [li for li, l in cls_labels if l == 0]
            pooled_output = outputs[1] # bsz x 1 x 768
            shuffled_pooled_output = shuffled_outputs[1]
            
            pooled_output[shuffle_labels, :, :] = shuffled_pooled_output[shuffle_labels, :, :]
            pooled_output = self.dropout(pooled_output)
        ...
        return start_loss, end_loss, ner_loss, cls_loss, output  # (loss), start_logits, end_logits, (hidden_states), (attentions)
